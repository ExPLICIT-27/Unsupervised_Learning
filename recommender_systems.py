...
top10_df = pd.read_csv("Unsupervised_Learning\data\content_top10_df.csv")
bygenre_df = pd.read_csv("Unsupervised_Learning\data\content_bygenre_df.csv")
print(top10_df)
print(bygenre_df)
...
#settting up cost function and optimizers to train the model
tf.random.set_seed(1)
...
#squared distance func for measuring movie similarity
def sq_dist(a, b):
    """
...
#precomputing some movie features
...
count = 50 #num movies to display
dim = len(vms)
...
m_dist = ma.masked_array(dist, mask=np.identity(dist.shape[0]))  # mask the diagonal (avoid showing distance between the same movie)
